...
def create_org(data: OrganizationCreate, db: Session = Depends(get_db_session), current_user: User = Depends(get_current_user)):
    return organization_service.create_organization(db, name=data.name, description=data.description)


@router.get("/", response_model=list[OrganizationOut])
def get_all(db: Session = Depends(get_db_session), current_user: User = Depends(get_current_user)):
    return organization_service.get_all_organizations(db)


@router.get("/{org_id}", response_model=OrganizationOut)
def get_one(org_id: UUID, db: Session = Depends(get_db_session), current_user: User = Depends(get_current_user)):
...
@router.put("/{org_id}")
def update_org(org_id: UUID, data: OrganizationUpdate, db: Session = Depends(get_db_session),
...
def delete_org(org_id: UUID, db: Session = Depends(get_db_session), current_user: User = Depends(get_current_user)):
...
def invite_user(org_id: UUID, payload: InviteUserDTO, db: Session = Depends(get_db_session),
                current_user: User = Depends(get_current_user)):
    return organization_service.invite_user_to_org(db, org_id, payload.user_id)


@router.delete("/{org_id}/remove/{user_id}")
def remove_user(org_id: UUID, user_id: UUID, db: Session = Depends(get_db_session),
                current_user: User = Depends(get_current_user)):
...
def suspend_user(org_id: UUID, user_id: UUID, db: Session = Depends(get_db_session),
                 current_user: User = Depends(get_current_user)):
    return organization_service.suspend_user(db, org_id, user_id)


@router.post("/{org_id}/unsuspend/{user_id}")
def unsuspend_user(org_id: UUID, user_id: UUID, db: Session = Depends(get_db_session),
                   current_user: User = Depends(get_current_user)):
